...
public:
	...
	void SetNDCDepth(cd::NDCDepth ndcDepth) { m_ndcDepth = ndcDepth; }
	cd::NDCDepth GetNDCDepth() const { return m_ndcDepth; }

	// View
	static cd::Vec3f GetLookAt(const cd::Transform& transform) { return transform.GetRotation().ToMatrix3x3() * cd::Vec3f(0.0f, 0.0f, 1.0f); }
	static cd::Vec3f GetUp(const cd::Transform& transform) { return transform.GetRotation().ToMatrix3x3() * cd::Vec3f(0.0f, 1.0f, 0.0f); }
	static cd::Vec3f GetCross(const cd::Transform& transform) { return transform.GetRotation().ToMatrix3x3() * cd::Vec3f(1.0f, 0.0f, 0.0f); }
